...
class BackTrack2(abstractSolver.AbstractSolver):
	...
	#fonction recursive, ajout des solutions petit a petit dans la liste "solution"			
        def back(self, problem, domains, solutions):
                ...
                if fini:
                        if problem.testSat(Node(domains)): # teste si le probleme verifie encore les contraintes (cast en noeud)
                        ...
                        for domainsBis in self.branch(domains):
                                self.back(problem,dict(domainsBis),solutions)#recopie pour eviter les probleme de dico communs dans la recursion
